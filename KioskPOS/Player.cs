	...
	public class Player
	{
		...
		public string MagStripe
		{
			set
			{
				...
				DbDataReader reader = xperdex.classes.StaticDsnConnection.KindExecuteReader( "select concat( concat( first_name, ' ' ), last_name ) from players_info where card='" + card + "'" );
				if( reader != null && reader.HasRows )
				{	
					reader.Read();
					_player_name = reader.GetString( 0 );
				}
				else
					_player_name = null;
