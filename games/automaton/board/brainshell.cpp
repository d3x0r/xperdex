...
public:
	...
	PPEICE_METHODS FindPeiceMethods( char *type );
	void InitMenus( void );
	void LoadPeices( char *file );
...
CTEXTSTR PickBoardName( PODBC odbc, int bMustExist )
{
	...
	if( frame )
	{
		...
		{
			PSI_CONTROL pc;
			if( pc = GetControl( frame, BTN_CREATENAME) )
			{
				if( bMustExist )
					HideCommon( pc );
				else
...
PPEICE_METHODS BRAINBOARD::FindPeiceMethods( char *type )
{
	PPEICE_METHODS methods = NULL;
	if( stricmp( type, "neuron" ) == 0 )
		methods =neuron_methods;
	else if( stricmp( type, "background" ) == 0 )
		methods =background_methods;
	else if( stricmp( type, "nerve" ) == 0 )
		methods =nerve_methods;
	else if( stricmp( type, "input" ) == 0 )
...
void BRAINBOARD::DefineABlockNoOpt( arg_list args )
{
	...
	if( image )
		NervePeice = board->CreateVia( "nerve", image, nerve_methods, (PTRSZVAL)this );
}
...
void BRAINBOARD::LoadPeices( char *file )
{
...
{
	{
		...
		//for( pbs = brain->first(); pbs; pbs = brain->next() )
		{
			...
			AppendPopupItem( comp_menu, MF_STRING|MF_POPUP, (_32)(menu = CreatePopup() ), "inputs" );
			...
			AppendPopupItem( comp_menu, MF_STRING|MF_POPUP, (_32)(menu = CreatePopup() ), "outputs" );
			AddLink( menus, (POINTER)menu );
			...
			AppendPopupItem( comp_menu, MF_STRING|MF_POPUP, (_32)(menu = CreatePopup() ), "module" );
			AddLink( menus, (POINTER)menu );
...
void BRAINBOARD::InitMenus( void )
{
	...
	AppendPopupItem( hMenu,MF_STRING|MF_POPUP, (_32)(hMenuComponents=CreatePopup()), "Add &Component" );
	{
		int n = 0;
		PBRAIN_STEM pbs;
		for( pbs = brain->first(); pbs; pbs = brain->next() )
	...
	{
		PMENU hPopup;
		AppendPopupItem( hMenu,MF_STRING|MF_POPUP, (_32)(hPopup = CreatePopup()), "Zoom" );
...
BRAIN brains[1] = { BRAIN( &clusters[0] ) };
...
char *board_layer_synapse_table =
