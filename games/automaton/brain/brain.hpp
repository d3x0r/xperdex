...
typedef struct connector *PCONNECTOR, CONNECTOR;
struct connector:public value{
   TEXTCHAR c_name[MAX_NAME_LEN]; // always make this last
	IMPORT CTEXTSTR name(void) { return c_name; }
	IMPORT connector():value() { strcpy( c_name, WIDE("unnamed") ); }
	IMPORT connector(CTEXTSTR conn_name, value *pvalue ):value(pvalue) { strcpy( c_name, conn_name ); }
	INDEX idx;
	PBRAIN_STEM pbs;
	...
	//};
#define CONNECTOR_CONSTRUCTOR( conn_type, type_name ) 	IMPORT connector( CTEXTSTR conn_name, conn_type *type_name ):value(type_name) { strcpy( connector::c_name, conn_name ); }
...
private:
   ...
   void DeleteComponents( void );
	...
	sack::timers::CRITICALSECTION cs;
	inline void Lock( void ) { sack::timers::EnterCriticalSec( &cs ); }
	inline void Unlock( void ) { sack::timers::LeaveCriticalSec( &cs ); }
...
public:
   ...
   //IMPORT _32 Load( FILE *File );
	...
	IMPORT void GetConnector( CTEXTSTR connector, struct connector **connector_result, int bInput );
};
#else
typedef void BRAIN;
...
#ifdef BRAIN_SOURCE
#define BRAIN_PROC(type,name) __declspec(dllexport) type CPROC name
#else
#define BRAIN_PROC(type,name) __declspec(dllimport) type CPROC name
#endif
#else
#ifdef BRAIN_SOURCE
#define BRAIN_PROC(type,name) type CPROC name
#else
#define BRAIN_PROC(type,name) extern type CPROC name
#endif
#endif
#endif
