...
#define VALUE_CONSTRUCTOR( val_type, name ) IMPORT value(val_type *name){ data.name=name; value::type=VAL_##name; }
	...
	IMPORT int null( void ) { return( type == VAL_NULL ); }
	IMPORT NATIVE get();
	IMPORT int get_int();
	...
	IMPORT void set( struct value *value );
	IMPORT value(){ set(VAL_UNDEFINED); }
