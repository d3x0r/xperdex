...
static PBRAIN CPROC CreateBrain( void )
{
	PBRAIN brain = new BRAIN;
   return brain;
}

static void CPROC DestroyBrain( PBRAIN brain )
{
	Release( brain );
}
...
void BRAIN::Process( void )
{
   if( bHalt )
      return;
   sack::timers::EnterCriticalSec( &cs );
	...
	sack::timers::LeaveCriticalSec( &cs );
}
...
#ifndef __WINDOWS__
_32 WriteFile( int h, POINTER p, _32 len, _32 *outlen, POINTER unused )
{
	_32 x;
	x = write( h, p, len );
   if( outlen )
...
void BRAIN::GetConnector( CTEXTSTR name, struct connector **connector_result, int bInput )
{
	CTEXTSTR start, end;
	start = name;
	end = strchr( start, '/' );
	...
	}
	{
		...
		{
			if( strncmp( pbs->name(), start, end-start ) == 0 )
			{
				break;
			}
		}
		if( pbs )
		{
			...
			while( end )
			{
				...
				{
					if( strncmp( actual->name(), start, end-start ) == 0 )
						break;
				}
				if( actual )
					pbs = actual;
			...
			if( !end )
			{
				if( bInput )
				{
					...
					{
						if( strncmp( conn->name(), start, end-start ) == 0 )
							break;
					}
