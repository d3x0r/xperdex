...
namespace OpenSkieScheduler3.Controls.Forms
{
	public partial class PriceEditor : Form
	{
		...
		SessionMacroTable session_macros;
		SessionTable sessions;
		PackGroupTable game_groups;
		...
		DataRow game_group;
		int prize_col_start;
		List<int> prize_levels;

		public PriceEditor()
		{
			prize_levels = new List<int>();
		...
		void dataGridView1_CellValueChanged( object sender, DataGridViewCellEventArgs e )
		{
			...
			try
			{
				...
				{
					if( IsPriceSame( Convert.ToInt32( session_game_group_game[SessionGame.PrimaryKey] ), Convert.ToInt32( prize_levels[e.ColumnIndex - prize_col_start] ), value ) )
					{
					}
					else
					{
						...
						if( radioButtonPriceMultiplier.Checked || radioButtonCompound.Checked )
						{
							ok = true;
							scalar_value = Convert.ToDouble( textBoxPriceIncrement.Text );
							if( radioButtonCompound.Checked )
								scalar_value = ( 100 + scalar_value ) / 100;
							else if( radioButtonPriceMultiplier.Checked )
								scalar_value = ( scalar_value ) / 100;
						}
						if( radioButtonPriceIncrement.Checked )
						{
							ok = true;
							scalar_value = (int)( new Money( textBoxPriceIncrement.Text ) );
						}
						for( n = 1; ( n + prize_col_start ) < dataGridView1.Columns.Count; n++ )
						{
							...
							if( ok 
								|| cell == null 
								//|| IsPriceBlank( Convert.ToInt32( session_game_group_game[SessionGameGroupGameOrder.PrimaryKey] ), Convert.ToInt32( prize_levels[n] ) )
								...
								
							{
								...
								else if( radioButtonPriceIncrement.Checked )
	...
	//			+ " and " + SessionPackOrder.PrimaryKey + "=" + this_game_group[GameGroupTable.PrimaryKey] );
		...
		bool IsPriceSame( int session_game_group_game_id, int prize_id, long prize )
		{
		...
		bool IsPriceBlank( int session_game_group_game_id, int prize_id )
		{
			// prize_data is a reflection of what the current values being displayed are.
			if( prize_data.Rows.Count > 0 )
			{
		...
		void UpdatePrice( int session_game_group_game_id, int prize_id, long prize )
		{
			if( prize_data.Rows.Count == 0 )
			{
				// no rows, this must be written as the default session value (even if it's a day override)
			save_default:
				dsn.ExecuteNonQuery( "insert into " + Names.schedule_prefix + "price_schedule (prize_level_id,session_game_group_game_id,session_number,session_macro_id,amount,session_id) values (" + prize_id
					+ "," + session_game_group_game_id
					+ ",0" 
			...
			else
			{
				...
				if( rows.Length == 0 )
				{
					// no other information about this, again, save this as the default.
					dsn.ExecuteNonQuery( "insert into " + Names.schedule_prefix + "price_schedule (prize_level_id,session_game_group_game_id,session_number,session_macro_id,amount,session_id) values (" + prize_id
						+ "," + session_game_group_game_id
						+ ",0"
						+ "," + session_macro[SessionMacroTable.PrimaryKey]
				...
				{
					// need to evaluate what sort of infomration this is, and whether it's the same...
					if( checkBox1.Checked )
					{
						// specific session override....
						if( rows[0]["session_prize"].GetType() == typeof( DBNull ) )
						{
							dsn.ExecuteNonQuery( "insert into " + Names.schedule_prefix + "price_schedule (prize_level_id,session_game_group_game_id,session_number,session_macro_id,amount,session_id) values (" + prize_id
								+ "," + session_game_group_game_id
								+ "," + session_macro_session[SessionDayMacroSessionTable.NumberColumn]
								+ "," + session_macro[SessionMacroTable.PrimaryKey]
								+ "," + (int)( prize )
								+ "," + session[SessionTable.PrimaryKey]
								+ ")"
						...
						else
						{
							...
							{
								if( rows[0]["day_prize"].GetType() == typeof( DBNull ) )
								{

								}
								else
								{
									// already is a day override... 
									dsn.ExecuteNonQuery( "insert into " + Names.schedule_prefix + "price_schedule (prize_level_id,session_game_group_game_id,session_number,session_macro_id,amount,session_id) values (" + prize_id
										+ "," + session_game_group_game_id
										+ "," + session_macro_session[SessionDayMacroSessionTable.NumberColumn]
										+ "," + session_macro[SessionMacroTable.PrimaryKey]
			...
			//if( day_of_week_type == -1 )
			{
				foreach( DataGridViewRow row in dataGridView1.Rows )
				{
					...
					for( level = 0; level < prize_levels.Count; level++ )
					{
						...
						if( prize == null )
							continue;
						if( IsPriceSame( Convert.ToInt32( session_game_group_game[SessionGame.PrimaryKey] ), Convert.ToInt32( prize_levels[level] ), prize ) )
							continue;

						UpdatePrice( Convert.ToInt32( session_game_group_game[SessionGame.PrimaryKey] ), Convert.ToInt32( prize_levels[level] ), prize );
						continue;
					}
				}
