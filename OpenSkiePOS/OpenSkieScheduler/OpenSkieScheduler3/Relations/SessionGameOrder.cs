...
using System.Data;
using OpenSkieScheduler.Relations.Meta;
using System.Windows.Forms;

namespace OpenSkieScheduler.Relations
{
	[SchedulePersistantTable]
	public class SessionGameOrder : MySQLRelationTable2<SessionTable, GameTable>
	{
		...
		public static String[,]		Relations = {
													...
													{"session_game_group_game_meta_game_group_info",	 GameGroupTable.PrimaryKey},
													{"session_game_group_game_meta_session_game",	     SessionGameRelation.PrimaryKey},
		...
		{
			if( e.Column.ColumnName == NumberColumn && e.Row.HasVersion( DataRowVersion.Original ) )
			{
				if( updating_number )
					return;
				DataRow[] these_rows = this.Select( "session_id=" + e.Row["session_id"], NumberColumn );
				...
				int oldval = Convert.ToInt32( e.Row[NumberColumn, DataRowVersion.Original] );
				int del = newval - oldval;
				if( ( row_index + 1 ) < these_rows.Length )
				if( Convert.ToInt32( these_rows[row_index + 1][NumberColumn] ) == newval )
				{
					...
					if( result == DialogResult.OK )
					{
						...
						{
							these_rows[row_index + n][NumberColumn] = Convert.ToInt32( these_rows[row_index + n][NumberColumn] ) + 1;
						}
						//this.EndLoadData();
				...
				if( ( row_index + 1 ) < these_rows.Length )
				if( Convert.ToInt32( these_rows[row_index + 1][NumberColumn] ) == ( oldval + 1 ) )
				{
					...
					if( result == DialogResult.OK )
					{
						...
						for( int n = 1; n < these_rows.Length - row_index; n++ )
						{
							these_rows[row_index + n][NumberColumn] = Convert.ToInt32( these_rows[row_index + n][NumberColumn] ) + del;
						}
						//this.EndLoadData();
						updating_number = false;
					}
		...
		public DataRow Current;// { get { return _Current; } }
	}
