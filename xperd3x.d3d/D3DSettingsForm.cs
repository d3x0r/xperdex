//-----------------------------------------------------------------------------
// TextureFile: D3DSettingsForm.cs
//
...
public class D3DSettings 
{
    ...
    /// <summary>
    /// The Depth stencils vertexFormat
    /// </summary>
	...
	/// <summary>
	/// Sets up the graphics settings with best available windowed mode, subject to 
	...
	public bool FindBestWindowedMode(bool doesRequireHardware, bool doesRequireReference,
									D3DEnumeration enumerationSettings,
	...
	EndWindowedDeviceComboSearch:
		...
		this.WindowedAdapterInfo = bestAdapterInfo;
		this.WindowedDeviceInfo = bestDeviceInfo;
		this.WindowedDeviceCombo = bestDeviceCombo;
		this.IsWindowed = true;
		this.WindowedDisplayMode = primaryDesktopDisplayMode;
		this.WindowedWidth = clientRectangle.Right - clientRectangle.Left;
		this.WindowedHeight = clientRectangle.Bottom - clientRectangle.Top;
		if (enumerationSettings.AppUsesDepthBuffer)
			this.WindowedDepthStencilBufferFormat = (DepthFormat)bestDeviceCombo.DepthStencilFormatList[0];
		this.WindowedMultisampleType = (MultiSampleType)bestDeviceCombo.MultiSampleTypeList[0];
		this.WindowedMultisampleQuality = 0;
		this.WindowedVertexProcessingType = (VertexProcessingType)bestDeviceCombo.VertexProcessingTypeList[0];
		this.WindowedPresentInterval = (PresentInterval)bestDeviceCombo.PresentIntervalList[0];

		return true;
	}

	/// <summary>
	/// Sets up the graphics settings with best available fullscreen mode, subject to 
	/// the doesRequireHardware and doesRequireReference constraints.  
	/// </summary>
	/// <param name="doesRequireHardware">Does the device require hardware support</param>
	/// <param name="doesRequireReference">Does the device require the ref device</param>
	...
	EndFullscreenDeviceComboSearch:
		...
		this.FullscreenAdapterInfo = bestAdapterInfo;
		this.FullscreenDeviceInfo = bestDeviceInfo;
		this.FullscreenDeviceCombo = bestDeviceCombo;
		this.IsWindowed = false;
		this.FullscreenDisplayMode = bestDisplayMode;
		if (enumerationSettings.AppUsesDepthBuffer)
			this.FullscreenDepthStencilBufferFormat = (DepthFormat)bestDeviceCombo.DepthStencilFormatList[0];
		this.FullscreenMultisampleType = (MultiSampleType)bestDeviceCombo.MultiSampleTypeList[0];
		this.FullscreenMultisampleQuality = 0;
		this.FullscreenVertexProcessingType = (VertexProcessingType)bestDeviceCombo.VertexProcessingTypeList[0];
		this.FullscreenPresentInterval = PresentInterval.Default;
		return true;
	}
...
public class D3DSettingsForm : System.Windows.Forms.Form
{
	...
	/// adapter vertexFormat list, resolution list, and refresh rate list.
	/// Updating the selected adapter vertexFormat will trigger updates of the 
	/// rest of the dialog.
	/// </summary>
	private void WindowedFullscreenChanged(object sender, System.EventArgs e)
	{
		...
		if (windowedRadioButton.Checked)
		{
			...
			// Update adapter vertexFormat combo box
	...
	/// <summary>
	/// Respond to a change of selected adapter vertexFormat by rebuilding the
	/// resolution list and back buffer vertexFormat list.  Updating the selected 
	/// resolution and back buffer vertexFormat will trigger updates of the rest 
	/// of the dialog.
	/// </summary>
	private void AdapterFormatChanged(object sender, System.EventArgs e)
	{
		...
		// Update backbuffer vertexFormat combo box
		GraphicsDeviceInfo deviceInfo = (GraphicsDeviceInfo)deviceComboBox.SelectedItem;
	...
	/// <summary>
	/// Respond to a change of selected back buffer vertexFormat by rebuilding
	/// the depth/stencil vertexFormat list, multisample type list, and vertex
	...
	/// <summary>
	/// Respond to a change of selected depth/stencil buffer vertexFormat.
	/// </summary>
