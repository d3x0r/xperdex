...
/// <summary>
/// Info about a depth/stencil buffer vertexFormat that is incompatible with a
/// multisample type
/// </summary>
...
/// <summary>
/// A combination of adapter vertexFormat, back buffer vertexFormat, and windowed/fullscreen 
...
public class D3DEnumeration
{
	...
	public int AppMinColorChannelBits = 5; // min color bits per channel in adapter vertexFormat
	public int AppMinAlphaChannelBits = 0; // min alpha bits per pixel in back buffer vertexFormat
		...
		foreach (Format adapterFormat in adapterFormatList)
		{
			foreach (Format backBufferFormat in backBufferFormatArray)
			{
				...
				foreach (bool isWindowed in isWindowedArray)
				{
					...
					// DeviceCombo that is supported by the system.  We still need to confirm that it'fromSurface 
					// compatible with the app, and find one or more suitable depth/stencil buffer vertexFormat,
	...
	public void BuildPresentIntervalList(GraphicsDeviceInfo deviceInfo, DeviceCombo deviceCombo)
	{
		...
		foreach (PresentInterval pi in piArray)
		{
			...
			// can'toTexture do a caps check for it -- it is always available.
