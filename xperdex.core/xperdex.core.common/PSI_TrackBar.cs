...
namespace xperdex.core
{
	...
	[ControlAttribute( Name = "PSI_TrackBar" )]
	public partial class PSI_TrackBar : Control
	{
		#region Events
		...
		[Description( "Set Slider bar width" )]
		[Category( "PSI_TrackBar" )]
		[DefaultValue( 30 )]
		public int BarWidth
		{
			get { return barWidth; }
		...
		private Color[ , ] aColorSchema = new Color[ , ]
        {
            ...
            {
                Color.Black, Color.LightGray, Color.Silver, Color.Red, Color.DarkRed, Color.Crimson, Color.Coral,
                Color.LightCoral
            },
            {
		...
		public ColorSchemas ColorSchema
		{
			...
			set
			{
				...
				barOuterColor = ( barOrientation == Orientation.Vertical ) ? aColorSchema[ sn, 6 ] : aColorSchema[ sn, 3 ];
				barInnerColor = ( barOrientation == Orientation.Vertical ) ? aColorSchema[ sn, 7 ] : aColorSchema[ sn, 4 ];
				barPenColor = aColorSchema[ sn, 5 ];
				elapsedOuterColor = ( barOrientation == Orientation.Vertical ) ? aColorSchema[ sn, 3 ] : aColorSchema[ sn, 6 ];
				elapsedInnerColor = ( barOrientation == Orientation.Vertical ) ? aColorSchema[ sn, 4 ] : aColorSchema[ sn, 7 ];
		...
		public PSI_TrackBar( int min, int max, int value )
		{
			InitializeComponent();
		...
		/// Initializes a new instance of the <see cref="ColorSlider"/> class.
		/// </summary>
		public PSI_TrackBar() : this( 0, 100, 50 ) { }
									 ...
									 Color barPenColorPaint, Color elapsedOuterColorPaint, Color elapsedInnerColorPaint )
		{
			try
			{
				...
				{
					int TrackY = ( ( ( barMaximum - trackerValue ) * ( ClientRectangle.Height - thumbSize ) ) / ( barMaximum - barMinimum ) );
				...
				if( barOrientation == Orientation.Horizontal )
				{
					...
					if( thumbCustomShape == null )
					{
						int temp = thumbRect.Height;
						thumbRect.Height = ( int )( barRect.Width * 1.4 );
						thumbRect.Y += ( temp - thumbRect.Height ) / 2;
					}
				...
				else
				{
					...
					if( thumbCustomShape == null )
					{
						int temp = thumbRect.Width;
						thumbRect.Width = ( int )( barRect.Width * 1.4 );
						thumbRect.X += ( temp - thumbRect.Width ) / 2;
					}
					thumbHalfRect.Width /= 2;
					elapsedRect = barRect;
					elapsedRect.Y = thumbRect.Top + thumbSize / 2;
					elapsedRect.Height = barRect.Height - elapsedRect.Y;
				}
			...
			if( Capture & e.Button == MouseButtons.Left )
			{
				...
				float coef = ( float )( barMaximum - barMinimum ) /
							 ...
							 ( ( barOrientation == Orientation.Horizontal ? ClientSize.Width : ClientSize.Height ) -
							  2 * margin );
				trackerValue = ( barOrientation == Orientation.Horizontal ) ?
					( int )( p * coef + barMinimum ) : barMaximum - ( int )( p * coef + barMinimum );
